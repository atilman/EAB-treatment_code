...
from ParamSet_middle_no_spillover import params_dict as pd0
...
titles = ['Policy ignores \nspillover benefits','Policy considers \nspillover benefits']
...
# Dynamics Figure 
fig, axs2 = plt.subplots(2, 2, figsize=(8, 7.66))
...
for i, ax in enumerate(axs2.flat):
    ...
    if i> 1:
        for j in range(len(labels1)):
            ax.plot(sol_mix[i-2].t, sol_mix[i-2].y[j], label=labels1[j],linestyle=linSty[j], linewidth=linWid[j], color=col[j], alpha=opacity[j])
        ax.set_title(titles[i-2],fontsize=textSize)  
        ax.set_xlabel('Time (years)',fontsize=textSize)
        ax.set_ylabel('Fraction of trees',fontsize=textSize)
        ax.text(0.02, 1.125, panelAlphas[i], transform=ax.transAxes, va='top', fontsize=textSize, fontweight='bold')
        if i==2:
...
"SAVE FIGS"
if saveFigs == True:
    fig.savefig("FIGS/FigSM8"+figName+".png", bbox_inches='tight',dpi=250)
